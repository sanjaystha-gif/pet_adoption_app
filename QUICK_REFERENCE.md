...
2. BACKEND_PROMPT.md - Copy for backend team (All required endpoints)
...
GET    /api/v1/bookings/pet/:petId      → Get bookings for pet
...
- Empty lists returned on error (graceful fallback)
...
### For User Bookings
1. Create `MyBookingsScreen` (code in guide)
...
| Admin approves | Status changes to 'approved', user sees it |
| Admin rejects | Status changes to 'rejected' with reason |
| User cancels | Status changes to 'cancelled' |
| Admin updates pet | New version appears on user dashboard |
...
A: Yes, optional enhancement. Add Socket.io listener for 'pet:updated' events

**Q: What if backend is slow?**
A: Frontend shows loading spinner, error message if timeout

**Q: How to handle duplicate bookings?**
A: Backend checks if booking already exists, returns error
...
### Frontend ✅ DONE
- [x] Booking entity/model
- [x] Booking service
- [x] Riverpod providers
- [x] Admin booking screen
- [x] Dialogs and forms
- [x] Error handling
- [x] Documentation
